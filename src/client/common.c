...
void check_error(cl_int errcode, const char* msg, ...) {
	...
	char formatted[1024];
	if (msg == NULL) {
	    fprintf(stderr, "Error %d: %s\n", errcode, clprobdistGetErrorString());
	}
	else {
	    va_list args;
	    va_start(args, msg);
	...
	bool echoVersion)
{
	...
	check_error(CLPROBDIST_INVALID_VALUE, "platform_index too large");
	...
	check_error(CLPROBDIST_INVALID_VALUE, "device_index too large");
	...
	const char* extra_options)
{
...
#endif
	    check_error(CLPROBDIST_OUT_OF_RESOURCES, "value of CLPROBDIST_ROOT too long");
    ...
    check_error(err, "cannot read source file\ncheck that the environment variable CLPROBDIST_ROOT set to the library root directory");
	
	//Append  CLPROBDIST_OBJ_MEM preprocessor
	cl_program program;
	if (preprocessors != NULL){

		char* NewSources = (char*)malloc(strlen(preprocessors) + 3 + strlen(sources[0]));

		strcpy(NewSources, preprocessors);
		strcat(NewSources, " \n");
		strcat(NewSources, sources[0]);

		program = clCreateProgramWithSource(context, 1, (const char**)&NewSources, NULL, &err);

		free(NewSources);
	}
	else 
		program = clCreateProgramWithSource(context, 1, (const char**)sources, NULL, &err);

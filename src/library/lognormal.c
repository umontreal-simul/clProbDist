...
#include "../include/clProbDist/lognormal.h"
...
#include "../include/clProbDist/private/lognormal.c.h"
...
clprobdistLognormal* clprobdistLognormalCreate(cl_double mu, cl_double sigma, size_t* bufSize, clprobdistStatus* err)
{
	...
	clprobdistLognormal* dist = (clprobdistLognormal*)malloc(sizeof(clprobdistLognormal));
	...
	if (bufSize)
		*bufSize = sizeof(clprobdistLognormal);
	if (err)
		*err = CLPROBDIST_SUCCESS;

	dist->contDistObj.decPrec = 15;
	dist->contDistObj.supportA = 0.0;
